...
def test_mean_difference_full_discrimination():
    ...
    y = [1, 1, 1, 1, 0, 0, 0, 0]
    s = [0, 0, 0, 0, 1, 1, 1, 1]
    assert metrics.mean_difference(y, s) == 1
    assert metrics.normalized_mean_difference(y, s) == 1
...
def test_mean_difference_partial_discrimination():
    ...
    y = [1, 1, 1, 1, 0, 0, 1, 1]
    s = [0, 0, 0, 0, 1, 1, 1, 1]
    assert metrics.mean_difference(y, s) == 0.5
    assert metrics.normalized_mean_difference(y, s) == 1
...
def test_mean_difference_no_discrimination():
    ...
    y = [0, 0, 1, 1, 0, 0, 1, 1]
    s = [0, 0, 0, 0, 1, 1, 1, 1]
    assert metrics.mean_difference(y, s) == 0
    assert metrics.normalized_mean_difference(y, s) == 0
