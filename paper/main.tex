...
\caption{
  ...
  % are shown for each of the three protected class contexts: \emph{female},
  % \emph{foreign worker}, and \emph{age below 25}. Left panels show training and
  % test \emph{AUC} scores (higher is better) of an L2-regularized Logistic
  % Regression model. The \(C\) parameter is the inverse regularization strength,
  % meaning that smaller values increase the regularization penalty. Model
  % complexity increases as \(C\) increases, leading to the overfitting regime at
  % \(C > 10^{-1}\). Right panels show training and test \emph{mean difference}
...
%     \centering
%     \includegraphics[width=6cm]{IMG/logistic_regression_validation_curve.png}
%     \caption{Arabic numerals}\label{fig:1a}
%   \end{subfigure}
%   \quad
%   \begin{subfigure}[t]{1in}
%     \centering
%     \includegraphics[width=3cm]{IMG/logistic_regression_fairness_utility_tradeoff.png}
